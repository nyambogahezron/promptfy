...
import { ArrowLeft, Bookmark, BookmarkCheck, Calendar, Copy, Tag } from "lucide-react-native";
...
export default function PromptDetailsScreen() {
	...
	return (
		<View style={[styles.container, { backgroundColor: colorScheme.background }]}>
			<View style={[styles.header, { backgroundColor: colorScheme.cardBackground }]}>
				<TouchableOpacity onPress={() => router.back()} style={styles.backButton}>
					<ArrowLeft size={24} color={colorScheme.text} />
				</TouchableOpacity>
				<Text style={[styles.headerTitle, { color: colorScheme.text }]}>Prompt Details</Text>
				<View style={styles.rightActions}>
					<TouchableOpacity style={styles.actionButton} onPress={() => toggleSave(prompt.id)}>
						{prompt.saved ? (
				...
				>
					<View style={styles.promptMeta}>
						<View style={styles.metaItem}>
							...
							<Text style={[styles.metaText, { color: colorScheme.secondaryText }]}>
					...
					<Text style={[styles.promptContent, { color: colorScheme.text }]}>{prompt.content}</Text>
				</Animated.View>
			</ScrollView>
		</View>
	);
