...
import { Check, Copy, Lightbulb, RefreshCw, Sparkles, Star, TrendingUp } from "lucide-react";
...
import type { AIEnhancement } from "@/types/prompt";
					...
					<CardContent>
						<div className="space-y-3">
							{enhancement.improvements.map((improvement, index) => (
								<motion.div
									key={`improvement-${index}-${improvement.slice(0, 50)}`}
						...
						<div className="space-y-4">
							...
							<div className="flex gap-2">
								<Button
									onClick={() =>
										enhancement.optimizedVersion &&
										onApplyOptimization(enhancement.optimizedVersion)
									}
									className="flex-1"
									disabled={!enhancement.optimizedVersion}
								>
								...
								<Button
									...
									onClick={() =>
										enhancement.optimizedVersion &&
										handleCopy(enhancement.optimizedVersion, "Optimized version")
									}
									className="transition-all duration-200"
									disabled={!enhancement.optimizedVersion}
								>
					...
					<CardContent>
						<div className="space-y-4">
							{enhancement.variations.map((variation, index) => (
								<motion.div
									key={`variation-${index}-${variation.slice(0, 50)}`}
									initial={{ opacity: 0, y: 20 }}
									animate={{ opacity: 1, y: 0 }}
									transition={{ delay: index * 0.1 }}
									className="space-y-3"
								>
									...
									<div className="bg-muted p-4 rounded-lg">
										<pre className="text-sm whitespace-pre-wrap font-mono">{variation}</pre>
									</div>
									<div className="flex gap-2">
										<Button
											variant="outline"
