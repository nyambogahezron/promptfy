...
class GeminiService {
	 ...
	 */
	async generatePrompt(userInstructions: string, template: string): Promise<string> {
		try {
...
`;
			...
			console.error("Error generating prompt with Gemini:", errorMessage);
	 ...
	 */
	async enhancePrompt(existingPrompt: string, enhancementRequest: string): Promise<string> {
		try {
			const model = this.client.getGenerativeModel({ model: this.model });
...
`;
			...
			const errorMessage = error instanceof Error ? error.message : String(error);
			console.error("Error enhancing prompt with Gemini:", errorMessage);
...
`;
			...
			const errorMessage = error instanceof Error ? error.message : String(error);
			console.error("Error analyzing prompt with Gemini:", errorMessage);
